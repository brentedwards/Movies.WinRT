...
namespace Movies.WinRT.Views
{
	...
	/// </summary>
	public sealed partial class MovieSearchResultsPage : Movies.WinRT.Common.LayoutAwarePage
	{
		...
		protected override void LoadState(Object navigationParameter, Dictionary<String, Object> pageState)
		{
			...
			var movieRepository = ServiceLocator.Current.GetInstance<IMovieRepository>();
			var allMovies = movieRepository.LoadMovies();
			_results = allMovies.Where(m => m.Name.ToLower().Contains(queryText.ToLower()));

			var genres = _results.GroupBy(m => m.Genre);

			var filterList = new List<Filter>();
			filterList.Add(new Filter("All", _results.Count(), true));
			foreach (var genre in genres)
			{
				filterList.Add(new Filter(genre.Key.ToString(), genre.Count()));
			}
		...
		void Filter_SelectionChanged(object sender, SelectionChangedEventArgs e)
		{
			...
			if (selectedFilter != null)
			{
				...
				{
					this.DefaultViewModel["Results"] = _results;
				}
				else
				{
					this.DefaultViewModel["Results"] = _results.Where(m => m.Genre.ToString() == selectedFilter.Name);
				}

				// Ensure results are found
				object results;
				IEnumerable<Movie> resultsCollection;
				if (this.DefaultViewModel.TryGetValue("Results", out results) &&
					(resultsCollection = results as IEnumerable<Movie>) != null &&
		...
		/// </summary>
		private sealed class Filter : Movies.WinRT.Common.BindableBase
		{
